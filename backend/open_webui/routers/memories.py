...
async def get_memory_query_embedding_prompt(request: Request) -> Optional[str]:
    prompt = os.getenv('MEMORY_QUERY_EMBEDDING_PROMPT')
    if prompt is not None:
        return prompt or None

    embedding_model = await Config.get('rag.embedding_model', '')
...
async def embed_memory_query(request: Request, content: str, user) -> list[float]:
    ...
    return await embedding_function(
        content,
        prefix=await get_memory_query_embedding_prompt(request),
        user=user,
    )
...
async def reset_memory_from_vector_db(
    ...
    vectors = await asyncio.gather(
        *[embed_memory_content(request, memory_vector_text(memory.content, memory.path), user) for memory in memories]
    )
