...
def _build_openai_file_upload_headers(
    ...
    token = None if auth_type == "none" else key
...
async def ensure_openai_file_id(
    ...
    *,
    ...
    if response.status_code >= 400:
        error = (
            response_payload.get("error", response_payload)
...
async def upload_file_handler(
    ...
    try:
        ...
        if process:
            ...
            file_item = await Files.get_file_by_id(file_item.id, db=db)
            if file_item:
