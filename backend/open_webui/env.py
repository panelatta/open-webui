...
CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS = os.getenv('CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS', '1')

if CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS == '':
    CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS = 1
else:
    try:
        CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS = int(CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS)
    except Exception:
        CHAT_RESPONSE_STREAM_RETRY_ATTEMPTS = 1


CHAT_RESPONSE_STREAM_RETRY_DELAY = os.getenv('CHAT_RESPONSE_STREAM_RETRY_DELAY', '0.5')

if CHAT_RESPONSE_STREAM_RETRY_DELAY == '':
    CHAT_RESPONSE_STREAM_RETRY_DELAY = 0.5
else:
    try:
        CHAT_RESPONSE_STREAM_RETRY_DELAY = float(CHAT_RESPONSE_STREAM_RETRY_DELAY)
    except Exception:
        CHAT_RESPONSE_STREAM_RETRY_DELAY = 0.5


CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT = os.getenv(
    'CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT',
    '1200',
)

if CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT == '':
    CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT = 1200
else:
    try:
        CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT = int(CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT)
    except Exception:
        CHAT_RESPONSE_STREAM_RETRY_VISIBLE_CHAR_LIMIT = 1200
...
ENABLE_RESPONSES_API_BACKGROUND_RESUME = (
    os.getenv('ENABLE_RESPONSES_API_BACKGROUND_RESUME', 'False').lower() == 'true'
)

RESPONSES_API_BACKGROUND_RESUME_MODEL_ALLOWLIST = {
    item.strip()
    for item in os.getenv('RESPONSES_API_BACKGROUND_RESUME_MODEL_ALLOWLIST', '').split(',')
    if item.strip()
}

RESPONSES_API_BACKGROUND_RESUME_BASE_URL_ALLOWLIST = {
    item.strip().rstrip('/')
    for item in os.getenv('RESPONSES_API_BACKGROUND_RESUME_BASE_URL_ALLOWLIST', '').split(',')
    if item.strip()
}

RESPONSES_API_BACKGROUND_RESUME_ATTEMPTS = os.getenv(
...
RESPONSES_API_CONTEXTUAL_RETRY_TOOL_ALLOWLIST = {
    item.strip()
    for item in os.getenv('RESPONSES_API_CONTEXTUAL_RETRY_TOOL_ALLOWLIST', '').split(',')
    if item.strip()
}
